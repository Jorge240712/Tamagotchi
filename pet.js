...
export class Pet {
  ...
  /**
   * Resuelve qué sprite debe mostrar la mascota.
   * tempSprite es seteado/limpiado exclusivamente por main.js.
   * _resolveSprite() solo lo respeta; NO lo limpia.
   */
  _resolveSprite() {
    // Siempre prioridad: muerto
    if (!this.alive) { this.currentSprite = 'dead'; return; }
    ...
    if (this.tempSprite) {
      this.currentSprite = this.tempSprite;
      return;
    }

    // 2. Durmiendo
    if (this.sleeping) { this.currentSprite = 'sleepy'; return; }

    // 3. Muy enfermo (salud crítica)
    if (this.health < 30) { this.currentSprite = 'sick'; return; }

    // 4. Triste por diversión baja
    if (this.fun < 50) { this.currentSprite = 'sad'; return; }

    // 5. Triste por salud baja (no crítica)
    if (this.health < 50) { this.currentSprite = 'sad'; return; }

    // 6. Malhumorado por disciplina baja
    if (this.discipline < 30) { this.currentSprite = 'grumpy'; return; }
